...
const CalendarV2 = () => {
  const { state, dispatch, getVisibleEvents } = useCalendar();
  ...
  const getEventCardColor = (color: string) => {
    // CalendarContext의 PASTEL_COLORS와 일치하는 매핑
    const pastelColorMap: { [key: string]: string } = {
      // 파스텔 톤 색상들 (CalendarContext의 PASTEL_COLORS와 동일)
      '#FFB3BA': 'text-red-800',         // 연한 핑크
      '#FFDFBA': 'text-orange-800',      // 연한 피치
      '#FFFFBA': 'text-yellow-800',      // 연한 옐로우
      '#BAFFC9': 'text-green-800',       // 연한 민트 그린
      '#BAE1FF': 'text-blue-800',        // 연한 스카이 블루
      '#E1BAFF': 'text-purple-800',      // 연한 라벤더
      '#FFE1BA': 'text-orange-700',      // 연한 오렌지
      '#FFBAD2': 'text-pink-800',        // 연한 로즈
      '#D2FFBA': 'text-lime-800',        // 연한 라임
      '#BAF2FF': 'text-cyan-800',        // 연한 아쿠아
      '#FFBAFF': 'text-fuchsia-800',     // 연한 매젠타
      '#D4EDDA': 'text-green-800',       // 연한 세이지 그린
      '#F8D7DA': 'text-red-700',         // 연한 코랄
      '#D1ECF1': 'text-teal-800',        // 연한 틸
      '#FFF3CD': 'text-yellow-800',      // 연한 크림
      '#E2E3E5': 'text-gray-800',        // 연한 실버
      '#FADBD8': 'text-red-700',         // 연한 블러쉬
      '#EBF5FB': 'text-blue-800',        // 연한 파우더 블루
      '#F4F4F4': 'text-gray-800',        // 연한 펄 화이트
      '#E8F8F5': 'text-emerald-800',     // 연한 에메랄드
      ...
      '#45b7d1': 'text-white',
      '#96ceb4': 'text-white',
      '#ffeaa7': 'text-gray-800',
      '#fd79a8': 'text-white',
      '#a29bfe': 'text-white',
      '#e17055': 'text-white',
      '#3B82F6': 'text-white',           // 기본 캘린더 색상
      '#EF4444': 'text-white',
      '#F59E0B': 'text-white',
      '#10B981': 'text-white',
      '#8B5CF6': 'text-white',
    ...
    // 파스텔 색상 매핑에서 텍스트 색상 찾기
    const textColor = pastelColorMap[color];
    if (textColor) {
      return textColor;
    }
    
    // 색상이 밝은지 어두운지 판단하여 텍스트 색상 결정 (hex 색상용)
